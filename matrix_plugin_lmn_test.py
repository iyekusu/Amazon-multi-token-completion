...
def create_mapping(dataset, model_name, dataset_path, model_path, dataset_suffix):
    ...
    pd.Series(mapping).to_csv(f'matrix_plugin_lmn_test/ext_dec_map_{model_name.replace("/", "_")}{dataset_suffix}.csv', index_label='phrase', header=['id'])
...
@torch.no_grad()
def load_sense_mapping(file_path):
    sense_map = {}
    with open(file_path, mode='r', encoding='utf-8') as file:
        reader = csv.DictReader(file)
        for row in reader:
            connective = row['Connective'].strip().lower()
            senses = json.loads(row['Senses'])
            sense_map[connective] = senses
    return sense_map

@torch.no_grad()
def test(matrix, dataset: datasets.DatasetDict, mapping: Dict, tokenizer: BertTokenizerFast, ckpt_path, sense_map,log=False):
    ...
    for irow, row in enumerate(tqdm(loader)):
        ...
        for i, span in enumerate(spans):
            gold_sense = row['sense1'][i]  # Accessing gold sense directly from the dataset
            predicted_connectives = [tokenizer.decode([rev_map.get(idx, '')]).strip() for idx in text_res[i][:max(acc_at_to_check)]]
            correct = any(sense_map.get(connective.lower(), []) for connective in predicted_connectives if gold_sense in sense_map.get(connective.lower(), []))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default='wiki_lmn_test')
    ...
    model_path = f'matrix_plugin_lmn_test/ext_dec_{model_name}{dataset_suffix}'
    dataset_path = f'data/matrix_plugin_lmn_test/seen_dataset_{model_name}{dataset_suffix}/'
    ...
    if args.force or not os.path.exists(model_path) or not os.path.exists(dataset_path):
        ...
        mapping = pd.read_csv(f'matrix_plugin_lmn_test/ext_dec_map_{model_name.replace("/", "_")}{dataset_suffix}.csv', na_filter=False).set_index('phrase')[
            'id'].to_dict()
    ...
    if args.test:
        ...
        if ckpt is None:
            ...
            ckpt = sorted(glob(f'matrix_plugin_lmn_test_results/{args.version}/checkpoints/*.ckpt'))[-1]
        print(ckpt)
        ...
        matrix.eval()
        sense_map = load_sense_mapping('MTC_sense.csv')
        test(matrix, seen_dataset, mapping, tok, best_ckpt, args.log, sense_map)


#with sense mapping
# with test dataset check and loading